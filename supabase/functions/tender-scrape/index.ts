  ...
  try {
    res = await fetchWithTimeout(baseUrl, {
      headers: {
        ...
        'User-Agent': 'Mozilla/5.0 (compatible; EngageHydrovacCRM-TenderBot/1.0)',
  ...
  try {
    res = await fetchWithTimeout(baseUrl, {
      headers: {
        ...
        'User-Agent': 'EngageHydrovacCRM-TenderBot/1.0',
      },
    }, 15000)
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err)
...
function buildDigestEmail(content: DigestContent): { subject: string; html: string } {
  ...
  const subject = `Engage Hydrovac weekly tender digest — ${newCount} new this week`
        ...
        <table role="presentation" width="640" cellpadding="0" cellspacing="0" style="background:#ffffff;border:1px solid #e5e7eb;border-radius:8px;overflow:hidden;max-width:640px;">
          ...
          <tr>
            <td style="padding:16px 28px 24px 28px;border-top:1px solid #e5e7eb;background:#f9fafb;">
              <p style="margin:0;font-size:11px;color:#6b7280;font-family:Arial,Helvetica,sans-serif;text-align:center;">Generated by Engage Hydrovac CRM · ${escapeHtml(content.generatedAt)}</p>
            </td>
          </tr>
        </table>
      </td>
