...
import { Home, Calendar, Briefcase, Users, Truck, ClipboardCheck, Building2, FileText, BarChart2, Settings, Menu, X, Bell } from "lucide-react";
      ...
      <aside className={cn(
          ...
          {navItems.map(item => {
            ...
            return (
              <Link key={item.to} to={item.to} onClick={() => setOpen(false)}
                className={cn("relative flex items-center gap-3 px-3 py-2.5 rounded-md text-sm mb-0.5 transition-colors",
                  active ? "bg-sidebar-accent text-amber-brand font-medium" : "text-sidebar-foreground/80 hover:bg-sidebar-accent hover:text-sidebar-foreground")}>
                {active && <span className="absolute left-0 top-1 bottom-1 w-1 rounded-r bg-amber-brand" />}
        ...
        <div className="p-3 border-t border-sidebar-border flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="text-[10px] uppercase tracking-wider font-mono text-amber-brand">Management</div>
          </div>
        </div>
      </aside>

      {open && <div onClick={() => setOpen(false)} className="lg:hidden fixed inset-0 top-11 bg-black/50 z-30" />}

      {/* Main */}
      <div className="flex-1 min-w-0 flex flex-col">
        <header className="h-16 border-b border-border bg-card flex items-center justify-between px-4 sm:px-6">
          <div className="flex items-center gap-3">
            <button onClick={() => setOpen(v => !v)} className="lg:hidden p-2 -ml-2 rounded-md hover:bg-accent">
              {open ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}
            </button>
            <div>
              <h1 className="text-lg sm:text-xl font-semibold tracking-tight">{title || "Dashboard"}</h1>
              <div className="text-xs text-muted-foreground font-mono hidden sm:block">Wed · 14 May 2025</div>
            </div>
          </div>
