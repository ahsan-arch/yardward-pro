...
const stats = [
  ...
  { label: "Pending Work Orders", value: "3", icon: ClipboardCheck, badge: "Needs review", tone: "warning" },
  { label: "Flagged Submissions", value: "1", icon: AlertTriangle, badge: "Urgent", tone: "danger" },
];
  ...
  return (
    <AdminShell title="Dashboard">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {stats.map(s => (
          <div key={s.label} className="bg-card border border-border rounded-lg p-4 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            <div className="flex items-start justify-between">
              <s.icon className="w-5 h-5 text-muted-foreground" />
              <span className={cn("text-[10px] font-mono uppercase tracking-wider px-1.5 py-0.5 rounded", toneClass[s.tone])}>{s.badge}</span>
            </div>
      ...
      <div className="mt-6 grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <div className="xl:col-span-2 bg-card border border-border rounded-lg shadow-[0_1px_3px_rgba(0,0,0,0.06)] overflow-hidden">
          <div className="flex items-center justify-between p-4 border-b border-border">
            ...
            <Link to="/admin/schedule" className="text-xs text-amber-brand hover:underline flex items-center gap-1">View all <ArrowUpRight className="w-3 h-3" /></Link>
          </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/40 text-xs uppercase tracking-wider text-muted-foreground">
                <tr>
                  ...
                  <th className="text-left font-medium px-4 py-2.5 hidden md:table-cell">Location</th>
              ...
              <tbody>
                {todays.map(j => (
                  <tr key={j.id} className="border-t border-border hover:bg-muted/30">
                    ...
                    <td className="px-4 py-3 hidden md:table-cell text-muted-foreground">{j.location}</td>
          ...
          <div className="relative pl-5">
            ...
            {activityFeed.map((e, i) => (
              <div key={i} className="relative pb-4 last:pb-0">
                <div className={cn("absolute -left-[14px] top-1 w-2.5 h-2.5 rounded-full ring-2 ring-card", dotColor[e.type])} />
